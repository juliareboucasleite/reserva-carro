...
export default function LandingPage({
    ...
    const protectionOptions = useMemo(() => {
        const groups = new Map();
        offers.forEach((offer) => {
            offer.protections.forEach((item) => {
    ...
    const mileageOptions = useMemo(() => {
        const groups = new Map();
        offers.forEach((offer) => {
            groups.set(offer.mileageLabel, (groups.get(offer.mileageLabel) || 0) + 1);
        });
        return [...groups.entries()].map(([label, count]) => ({ label, count }));
    }, [offers]);
    ...
    const paymentOptions = useMemo(() => {
        const groups = new Map();
        offers.forEach((offer) => {
            groups.set(offer.payment, (groups.get(offer.payment) || 0) + 1);
        });
        return [...groups.entries()].map(([label, count]) => ({ label, count }));
    }, [offers]);
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3">
                            <div className="min-w-0">
                                <p className="text-lg font-bold text-ink">
                                    {pickupLocationData?.label || pickupLocation || 'Portugal'}{' '}
                                    <span className="px-2 text-muted">›</span>{' '}
                                    {returnLocationData?.label ||
                                        returnLocation ||
                                        pickupLocationData?.label ||
                                        pickupLocation ||
                                        'Portugal'}
                                </p>
                            ...
                            <div className="overflow-hidden rounded-[28px] border border-border bg-paper shadow-sm">
                                <div className="flex items-center justify-between border-b border-border-soft px-5 py-4">
                                    ...
                                    <button
                                        ...
                                        className="text-sm font-medium text-[#0f6bdf] transition hover:text-[#0a55b4]"
                                    >
                                        Limpar filtros
                                    </button>
                                </div>
                                        ...
                                        <div className="space-y-2">
                                            {pickupModeOptions.map((option) => (
                                                <SidebarCheckbox
                                                    ...
                                                    onChange={() =>
                                                        toggleFilterValue(setSelectedPickupModes, option.label)
                                                    }
                                                />
                                            ))}
                                        </div>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {featureOptions.map((option) => (
                                                <button
                                                    key={option.label}
                                                    type="button"
                                                    onClick={() => toggleFilterValue(setSelectedFeatures, option.label)}
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {ratingOptions.map((option) => (
                                                <button
                                                    key={option.label}
                                                    type="button"
                                                    onClick={() => toggleFilterValue(setSelectedRatings, option.label)}
                                                    className={`rounded-full border px-3 py-2 text-sm transition ${
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {protectionOptions.map((option) => (
                                                <button
                                                    ...
                                                    type="button"
                                                    onClick={() =>
                                                        toggleFilterValue(setSelectedProtections, option.label)
                                                    }
                                                    className={`rounded-full border px-3 py-2 text-sm transition ${
                                                        selectedProtections.includes(option.label)
                                                            ? 'border-sky-500 bg-sky-50 text-sky-900'
                                                            : 'border-border bg-paper-2 text-ink hover:border-ink/20'
                                                    }`}
                                                >
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {includedOptions.map((option) => (
                                                <button
                                                    key={option.label}
                                                    type="button"
                                                    onClick={() => toggleFilterValue(setSelectedIncluded, option.label)}
                                                    className={`rounded-full border px-3 py-2 text-sm transition ${
                                                        selectedIncluded.includes(option.label)
                                                            ? 'border-sky-500 bg-sky-50 text-sky-900'
                                                            : 'border-border bg-paper-2 text-ink hover:border-ink/20'
                                                    }`}
                                                >
                                        ...
                                        <div className="space-y-2">
                                            {mileageOptions.map((option) => (
                                                <SidebarCheckbox
                                                    ...
                                                    onChange={() =>
                                                        toggleFilterValue(setSelectedMileage, option.label)
                                                    }
                                                />
                                            ))}
                                        </div>
                                        ...
                                        <div className="space-y-2">
                                            {paymentOptions.map((option) => (
                                                <SidebarCheckbox
                                                    ...
                                                    onChange={() =>
                                                        toggleFilterValue(setSelectedPayments, option.label)
                                                    }
                                                />
                                            ))}
                                        </div>
                            ...
                            <div className="mb-6 flex gap-3 overflow-x-auto pb-2">
                                {categoryOptions.map((option) => (
                                    <button
                                        key={option.label}
                                        type="button"
                                        onClick={() => toggleFilterValue(setSelectedCategories, option.label)}
                                        className={`min-w-[170px] rounded-[24px] border bg-paper px-4 py-4 text-left shadow-sm transition ${
                            ...
                            <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
                                <div>
                                    ...
                                    <h2 className="mt-4 text-4xl font-bold tracking-tight text-ink">
                                        {filteredOffers.length} carro{filteredOffers.length === 1 ? '' : 's'} encontrados
                                    </h2>
                                </div>
                                ...
                                <div className="mb-5 flex flex-wrap items-center gap-2">
                                    ...
                                    {activeFilterTags.map((tag) => (
                                        <button
                                            ...
                                            className="rounded-full border border-sky-200 bg-sky-50 px-3 py-2 text-sm font-medium text-sky-900"
                                        >
                                            {tag.value} ×
                                        </button>
                                    ))}
                                    <button
                                        type="button"
                                        onClick={clearSearchFilters}
                                        className="ml-2 text-sm font-medium text-[#0f6bdf] transition hover:text-[#0a55b4]"
                                    >
                                        Limpar filtros
                                    </button>
                                </div>
            ...
            <div className="flex flex-1 flex-col gap-3 p-4">
                ...
                {vehicle.activeReservation && vehicle.availability !== 'available' && vehicle.availability !== 'inoperational' && (
                    <p className="rounded-md bg-warn-soft px-2.5 py-1.5 text-[11px] text-warn">
                        {[
                            vehicle.activeReservation.requesterName,
                            vehicle.activeReservation.endDate &&
                            vehicle.activeReservation.endDate !== vehicle.activeReservation.startDate
                                ? `${vehicle.activeReservation.startDate} → ${vehicle.activeReservation.endDate}`
                                : vehicle.activeReservation.startDate,
                        ]
                            .filter(Boolean)
                            .join(' · ')}
                    </p>
                    ...
                    <div className="mt-5 grid gap-5 xl:grid-cols-[1fr_220px]">
                        ...
                        <div className="rounded-2xl border border-border-soft bg-paper-2/60 p-4">
                            ...
                            <button
                                type="button"
                                onClick={onReserve}
                                disabled={offer.vehicle.availability === 'inoperational'}
                                className="mt-5 w-full rounded-2xl bg-[#17894e] px-4 py-3 text-sm font-semibold text-white transition hover:bg-[#117241] disabled:cursor-not-allowed disabled:bg-slate-300"
                            >
                                Continuar
                            </button>
                        </div>
...
function SpecPill({ children }) {
    return (
        <span className="rounded-xl bg-paper-2 px-3 py-2 text-sm font-medium text-ink">
            {children}
        </span>
    );
}
